...
interface SearchFormProps {
  initialValues: { tabPane: Portfolio; selectOption: ClientList };
...
export const ClientDetailsInfo: FunctionComponent<SearchFormProps> = (
  ...
  const [selectOption, setSelectOption] = useState<ClientList>(
  ...
  const sellOrders = async () => {
    ...
    try {
      ...
      const correlationId = selectOption.productDetailsList.filter(
        ...
        const bodyValue = {
          message: {
            ...
            payload: {
              ...
              portfolioName: selectOption.name,
              openId: [selectOption.openId],
      ...
      {selectOption && (
        <div className={styles.rowbox}>
          ...
          <Row gutter={8} justify={'space-around'}>
            <Col>客户名称：{selectOption?.name}</Col>
            <Col>
              跟投产品：
            ...
            <Col>
              创建日期：{formatDateWithTooltip(selectOption?.createdTime)}
            </Col>
            <Col>
              到期日期：
              {formatDateWithTooltip(
                selectOption?.productDetailsList.find(
